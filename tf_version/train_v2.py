...
import optimization_v2 as optimization  # _freeze as optimization
...
# tf.config.set_soft_device_placement = False  # 用来设置是否在特殊情况下在cpu上进行计算
...
config = {
    ...
    "bert_config": "/Users/wangxs/repository/NLP/bert/chinese_L-12_H-768_A-12/bert_config.json",  # bert模型配置文件
    "init_checkpoint": "/Users/wangxs/repository/NLP/bert/chinese_L-12_H-768_A-12/bert_model.ckpt",  # 预训练bert模型
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, labels, keep_prob, num_labels,
    ...
    with tf.variable_scope("loss"):
        ...
        input_m = tf.math.count_nonzero(input_mask, -1)

        log_likelihood, transition_matrix = tfa.text.crf_log_likelihood(
            logits, labels, input_m)
        loss = tf.reduce_mean(input_tensor=-log_likelihood)
        # inference
        viterbi_sequence, viterbi_score = tfa.text.crf_decode(
            logits, transition_matrix, input_m)
        # 不计算 padding 的 acc
        equals = tf.reduce_sum(
            input_tensor=tf.cast(tf.equal(tf.cast(viterbi_sequence, tf.int64), labels), tf.float32) * tf.cast(input_mask,
                                                                                                 tf.float32))
        acc = equals / tf.cast(tf.reduce_sum(input_tensor=input_mask), tf.float32)
...
def main():
    ...
    train_op, learning_rate = optimization.create_optimizer(
        total_loss, learning_rate_init, num_train_steps * config["num_train_epochs"], num_warmup_steps, False)
    ...
    with tf.Session() as sess:
        ...
        def train_step(ids, mask, segment, y, step):
            feed = {input_ids: ids,
                    ...
                    labels: y,
                    keep_prob: 0.9}
            _, lr, out_loss, acc_, p_ = sess.run([train_op, learning_rate, total_loss, acc, probabilities], feed_dict=feed)
            ...
            print("step :{}, lr:{}, loss :{}, acc :{}".format(step, lr, out_loss, acc_))
            ...
            for i in range(num_train_steps):
                ...
                if step % eval_per_step == 0 and step >= config["eval_start_step"]:
                    ...
                    for j in range(num_dev_steps):  # 一个 epoch 的 轮数
                        ...
                        out_loss, pre, y = dev_step(ids_dev, mask_dev, segment_dev, y_dev, step)
